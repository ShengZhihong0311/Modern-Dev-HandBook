...
### 隔离技术对比：虚拟机、虚拟环境、容器、本地

开发者经常混淆"虚拟机"、"虚拟环境"、"容器"这些概念。它们都涉及"隔离"，但隔离的层级和目的完全不同。

**四种隔离技术对比**：

| 技术 | 隔离层级 | 隔离内容 | 典型用途 | 资源开销 |
|------|----------|----------|----------|----------|
| **本地（无隔离）** | 无 | 无 | 个人开发、快速实验 | 最低 |
| **虚拟环境（.venv）** | Python 包级别 | Python 依赖 | 项目依赖隔离 | 极低 |
| **容器（Docker）** | 进程级别 | 文件系统 + 依赖 + 网络 | 环境一致性、部署 | 低 |
| **虚拟机（VM）** | 操作系统级别 | 整个操作系统 | 完全隔离、多系统 | 高 |
...
│  │  └── 所有项目共享 → 依赖冲突风险高                                   ││
│  └─────────────────────────────────────────────────────────────────────┘│
│  风险：项目 A 需要 numpy 1.26，项目 B 需要 numpy 2.0 → 冲突              │
├─────────────────────────────────────────────────────────────────────────┤
│  虚拟环境（.venv）                                                       │
│  ┌─────────────────────────────────────────────────────────────────────┐│
│  │  操作系统                                                            ││
│  │  ├── 系统 Python                                                    ││
│  │  ├── 项目 A/.venv/ → numpy 1.26                                     ││
│  │  └── 项目 B/.venv/ → numpy 2.0                                      ││
│  └─────────────────────────────────────────────────────────────────────┘│
│  隔离：仅 Python 包，共享操作系统和系统库                                │
│  用途：同一台机器上管理多个项目的依赖                                     │
├─────────────────────────────────────────────────────────────────────────┤
│  容器（Docker）                                                          │
│  ┌───────────────────────┐  ┌───────────────────────┐                   │
...
很多 Windows 开发者会困惑：我已经用了 WSL，为什么还要用 `.venv`？这不是重复吗？
...
┌─────────────────────────────────────────────────────────────────────────┐
│  Windows 主系统                                                         │
│  ├── Windows 应用（VS Code、浏览器等）                                   │
│  └── WSL 子系统（Ubuntu）                                               │
│      ├── 系统 Python（apt install python3）                             │
│      │   └── 全局包（pip install --break-system-packages）              │
│      │       └── 风险：污染系统环境，多项目冲突                          │
│      │                                                                  │
│      ├── 项目 A/                                                        │
│      │   └── .venv/  ← 隔离项目 A 的 Python 依赖                        │
│      │       ├── Python 解释器（链接到系统 Python）                      │
│      │       └── 项目 A 的包（numpy 1.26, fastapi 等）                  │
│      │                                                                  │
│      └── 项目 B/                                                        │
│          └── .venv/  ← 隔离项目 B 的 Python 依赖                        │
│              ├── Python 解释器（链接到系统 Python）                      │
│              └── 项目 B 的包（numpy 2.0, django 等）                    │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 层级 | 隔离什么 | 为什么需要 |
|------|----------|------------|
| **WSL** | 隔离操作系统 | 让 Windows 用户获得 Linux 开发环境 |
| **.venv** | 隔离 Python 包 | 让同一 WSL 中多个项目依赖互不干扰 |

**常见误区**：

| 误区 | 正确理解 |
|------|----------|
| "WSL 就是虚拟环境" | WSL 是操作系统级隔离，.venv 是 Python 包级隔离，两者互补 |
| "用 WSL 就不需要 .venv" | 错误。WSL 中多个项目仍会依赖冲突，必须用 .venv |
| "虚拟机和容器一样" | 虚拟机隔离完整操作系统，容器只隔离进程和文件系统 |
...
# 2. 用 uv 创建虚拟环境并安装依赖
uv sync                    # 创建 .venv/ 并安装 pyproject.toml 中的依赖

# 3. 验证虚拟环境隔离
uv run python -c "import sys; print(sys.prefix)"
# 输出应指向 .venv 目录，而非系统 Python

# 4. 验证包隔离
uv pip list                # 只显示项目依赖，不显示系统全局包
